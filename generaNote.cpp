...
int generateUniqueNote () {
	...
	while (!isUnique) {
		...
		for (int i=0; i<noRepeat; ++i) {
			// std::cerr << i << " cheching " << latestNotes[i] << " against " << aNote << std::endl;
...
//	int timerSeed = 0xcaffe;  // and not time(NULL)
	...
	if (argc!=5) {
		std::cerr << "Syntax: " << argv[0] << " numeroNote timerSeed noRepeat [name|note]" << std::endl;
		return -1;
	}
	int nNotes = atoi(argv[1]);
        int timerSeed = atoi(argv[2]);
	noRepeat = atoi(argv[3]);
	if (noRepeat<0) noRepeat=0;
	if (noRepeat>6) noRepeat=6;
	...
	for (int i=0; i<nNotes; ++i) {
		...
		if (noRepeat) latestNotes[i%noRepeat] = nota;
	}
	std::cout << std::endl;
	return 0;
}
